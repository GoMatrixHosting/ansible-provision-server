...
if [ "$TARIT" = 1 ]; then
  tar --exclude='./synapse/storage/media-store/remote_content' -czf /chroot/export/matrix.tar.gz ./awx ./synapse
fi
...
echo "$DATE_TIME $0: Snapshot (returned $DOCKERRC at $DOCKER_ELAPSED seconds)+ TAR (returned $TARRC at $TAR_ELAPSED seconds) completed, database.gz size: $FILE_SIZE" >> /matrix/awx/export.log
